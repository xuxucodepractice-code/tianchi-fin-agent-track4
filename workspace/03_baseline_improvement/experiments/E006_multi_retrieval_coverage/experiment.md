...
- 本轮固定 `pair_id=e006-development-pair-01`：treatment 启动前必须重新验证 control，
  并把 control 的 run ID 以及 observations/manifest/receipt 哈希写入自身 Trace；control
  目录同时原子生成一次性 `treatment_claim.json`，禁止 treatment-first 或同一 control
  的正常重复运行。
- 信任边界：这是内部可审计实验，不是带服务端签名的第三方密码学证明。主理人承诺
  不删除、搬移、替换或回滚固定的两臂目录与 treatment claim；若主动破坏这些本地锁，
  需另行引入远端不可回滚见证，当前实验不得声称 one-shot。
