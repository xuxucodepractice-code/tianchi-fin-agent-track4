...
`DEVELOPMENT_NO_GO / TREATMENT_ACCURACY_REGRESSION / DO_NOT_SUBMIT`。
...
fresh treatment 随后也完整通过：13/13、52 logical/physical attempts、零 retry、
180,967 tokens、served model=`qwen-plus`。严格 raw audit 证明 treatment 未知、越界、
重复 EV ref 均为 0，两臂 retrieval/evidence 逐字节相同，Trace/temporal/schema 全部
PASS。然而准确率 control 9/13、treatment 8/13；`fc_a_001` 从正确 `ABD` 回退为 `AD`，
同时触发父版本正确题零回退门。结论为 development NO-GO，不运行 prospective。
