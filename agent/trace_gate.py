...
E002_PIPELINE_VERSION = "v2s2"
E002_EXPERIMENT_ID = "E002"
E002_MODEL = "qwen-plus"
E002_TOP_K = 5
E002_SELECTION_SHA256 = (
    "662cb1694c8e2b87ce98f96f04604da0f141b2c76a213eeb0b4812d3438e80f5"
)
...
def candidate_experiment_policy_errors(
    config: dict[str, Any],
    *,
    model: str,
    base_url: str,
) -> list[str]:
    """锁死已预注册计分实验的单变量签名；其他版本暂不施加 E002 规则。"""
    experiment_id = str(config.get("experiment_id", ""))
    pipeline_version = str(config.get("pipeline_version", ""))
    if experiment_id != E002_EXPERIMENT_ID and pipeline_version != E002_PIPELINE_VERSION:
        return []

    errors: list[str] = []
    expected = {
        "experiment_id": E002_EXPERIMENT_ID,
        "pipeline_version": E002_PIPELINE_VERSION,
        "top_k": E002_TOP_K,
        "hide_doc_ids": False,
    ...
    for field, value in expected.items():
        if config.get(field) != value:
            errors.append(
                f"E002 requires {field}={value!r}, got {config.get(field)!r}"
            )
    if model != E002_MODEL:
        errors.append(f"E002 requires model={E002_MODEL!r}, got {model!r}")
    if base_url.rstrip("/") != OFFICIAL_DASHSCOPE_BASE_URL:
        errors.append("E002 requires the official DashScope base_url")
    return errors


def assert_candidate_experiment_policy(
    config: dict[str, Any],
    *,
    model: str,
    base_url: str,
) -> None:
    """在首次 API 调用前拒绝偏离预注册签名的候选运行。"""
    errors = candidate_experiment_policy_errors(
...
class AgentTraceRecorder:
    ...
    def record_derivation(self, result: dict[str, Any]) -> None:
        ...
        mcq_comparison = result.get("mcq_comparison")
        if isinstance(mcq_comparison, dict):
            comparison_ids = [
                *(
                    mcq_comparison.get("trace_call_ids", [])
                    if isinstance(mcq_comparison.get("trace_call_ids"), list)
                    else []
                ),
                mcq_comparison.get("trace_call_id"),
            ]
            call_ids.extend(
                str(value)
...
def validate_trace_directory(
    ...
    *,
    ...
    if require_candidate_eligible:
        ...
        errors.extend(
            candidate_experiment_policy_errors(
                manifest.get("config") or {},
                model=str(model_identity.get("model", "")),
                base_url=str(model_identity.get("base_url", "")),
            )
        )
    ...
    for index, item in enumerate(derivations, start=1):
        ...
        if not isinstance(derivation, dict) or not derivation.get("method"):
            ...
            try:
                ...
                elif method == MCQ_GLOBAL_METHOD:
                    valid_options = derivation.get("valid_options")
                    if valid_options != ["A", "B", "C", "D"]:
                        raise ValueError("MCQ global valid_options must be A/B/C/D")
                    input_choice = str(derivation.get("input_choice", ""))
                    comparison = derivation.get("input_comparison")
                    if not isinstance(comparison, dict):
                        raise ValueError("MCQ global input_comparison missing")
                    if str(comparison.get("answer", "")) != input_choice:
                        raise ValueError("MCQ global input_choice differs from comparison")
                    reference_judgments = derivation.get("reference_judgments")
                    if not isinstance(reference_judgments, dict) or sorted(
                        reference_judgments
                    ) != valid_options:
                        raise ValueError("MCQ global reference judgments must cover A/B/C/D")
                    if require_candidate_eligible and any(
                        not isinstance(value, dict)
                        or value.get("judgment") == "error"
                        or bool(value.get("error"))
                        for value in reference_judgments.values()
                    ):
                        errors.append(
                            f"{prefix}: candidate MCQ reference judgment contains errors"
                        )
                    if require_candidate_eligible and (
                        bool(derivation.get("parse_error"))
                        or derivation.get("fallback_used") is not False
                        or input_choice not in valid_options
                    ):
                        errors.append(
                            f"{prefix}: candidate MCQ comparison is invalid or used fallback"
                        )
                    replay = normalize_mcq_global_choice(
                        input_choice,
                        {key: "" for key in valid_options},
                        fallback_answer=str(
                            derivation.get("fallback_answer", valid_options[0])
                        ),
                    )
                    if replay.get("fallback_used") != derivation.get("fallback_used"):
                        errors.append(f"{prefix}: MCQ fallback_used differs on replay")
                else:
        ...
        configured_pipeline = str((manifest.get("config") or {}).get("pipeline_version", ""))
        if (
            require_candidate_eligible
            and configured_pipeline == "v2s2"
            and str(item.get("answer_format", "")) == "mcq"
            and method != MCQ_GLOBAL_METHOD
        ):
            errors.append(f"{prefix}: v2s2 MCQ must use the M1 global normalizer")
        if method == "agent.normalize_answer.normalize_answer":
            ...
            derivation = item.get("answer_derivation") or {}
            reference_judgments = derivation.get("reference_judgments") or {}
            if len(qid_calls) != 5:
                errors.append(f"{prefix}: M1 requires exactly five traced calls")
            for option_key, expected in reference_judgments.items():
                matches = [
                    call
                    for call in qid_calls
                    if str((call.get("context") or {}).get("option_key", ""))
                    == str(option_key)
                    and (
                        str((call.get("context") or {}).get("stage", ""))
                        == "option_independent_judgment"
                        if require_candidate_eligible
                        else str((call.get("context") or {}).get("stage", "")).endswith(
                            "option_independent_judgment"
                        )
                    )
                ]
                if len(matches) != 1:
                    errors.append(
                        f"{prefix}: expected exactly one raw reference call for option {option_key}"
                    )
                    continue
                raw_content = str(matches[0].get("response_content", ""))
                raw_object = extract_json_from_text(raw_content)
                parsed_reference, reference_error = _parse_judgment(
                    raw_content, str(option_key)
                )
                if (
                    not isinstance(raw_object, dict)
                    or str(raw_object.get("option", "")) != str(option_key)
                    or reference_error
                    or parsed_reference.get("judgment")
                    != (expected or {}).get("judgment")
                    or parsed_reference.get("evidence_refs", [])
                    != (expected or {}).get("evidence_refs", [])
                    or any(
                        ref < 1
                        or ref > len(matches[0].get("model_evidence") or [])
                        for ref in parsed_reference.get("evidence_refs", [])
                    )
                    or bool((expected or {}).get("error"))
                ):
                    errors.append(
                        f"{prefix}: reference option {option_key} differs from raw response"
                    )
            comparison_calls = [
                call
                for call in qid_calls
                ...
                and not (call.get("context") or {}).get("option_key")
            ]
            if len(comparison_calls) != 1:
                errors.append(f"{prefix}: expected exactly one raw MCQ comparison call")
            else:
                parsed_comparison, parse_error = parse_mcq_global_comparison_response(
                    str(comparison_calls[0].get("response_content", "")),
                    ["A", "B", "C", "D"],
                )
                if (
                    parsed_comparison != derivation.get("input_comparison")
                    or str(parse_error or "")
                    != str(derivation.get("parse_error") or "")
                ):
                    errors.append(f"{prefix}: MCQ derivation differs from raw comparison")
    ...
    if artifact_dir is not None:
        ...
        try:
            ...
            for derivation in derivations:
                ...
                answer_derivation = derivation.get("answer_derivation") or {}
                if str(answer_derivation.get("method", "")) == MCQ_GLOBAL_METHOD:
                    comparison = record.get("mcq_comparison")
                    expected_comparison = answer_derivation.get("input_comparison")
                    if not isinstance(comparison, dict) or {
                        ...
                        errors.append(
                            f"artifact evidence MCQ comparison differs from trace: {qid}"
                        )
                    comparison_calls = [
                        call
                        for call in calls
                        if str(call.get("call_id", ""))
                        in set(map(str, derivation.get("trace_call_ids") or []))
                        and str((call.get("context") or {}).get("stage", ""))
                        == "mcq_global_comparison"
                    ]
                    expected_model_evidence = _flatten_mcq_model_evidence(
                        {"options": record.get("retrieval", {})}
                    )
                    if (
                        len(comparison_calls) != 1
                        or comparison_calls[0].get("model_evidence")
                        != expected_model_evidence
                    ):
                        errors.append(
                            f"artifact retrieval differs from MCQ model evidence: {qid}"
                        )
