"""Independently audit immutable E010 churn with correct integer-zero handling."""
...
from agent.run_e010_prospective_arm import (
    ...
    TECHNICAL_REPLAY_RESULT_PATH,
    CONTROL_REFERENCE_PATH,
    FROZEN_INPUT_SHA256,
    PAIR_ID,
    PRIMARY_CLAIM_PATH,
    PROSPECTIVE_LABELS_PATH as E010_LABELS_PATH,
    PROSPECTIVE_SCORED_RESULT_PATH as E010_SCORED_RESULT_PATH,
    ...
    OFFLINE_GATE_PATH,
    TLS_CA_BUNDLE_PATH,
    CHURN_REPORT_PATH as E010_FROZEN_CHURN_PATH,
...
E011_EXPERIMENT_DIR = (
    REPO_ROOT
    / "workspace/03_baseline_improvement/experiments/E011_e010_churn_zero_value_audit"
)
E011_OUTPUT_ROOT = REPO_ROOT / "outputs/experiments/E011_e010_churn_zero_value_audit"
E011_AUDIT_ID = "e011-e010-churn-audit-01"
E011_RUN_FREEZE_PATH = E011_EXPERIMENT_DIR / "audit_run_freeze.json"
E011_INPUT_MANIFEST_PATH = E011_EXPERIMENT_DIR / "frozen_input_manifest.json"
E011_AUDIT_CLAIM_PATH = E011_OUTPUT_ROOT / "audit_claim.json"
E011_AUDIT_REPORT_PATH = E011_OUTPUT_ROOT / "e010_pair_churn_audit.json"
E011_LABELS_PATH = E011_EXPERIMENT_DIR / "prospective_labels.json"
E011_SCORED_RESULT_PATH = E011_EXPERIMENT_DIR / "prospective_scored_result.json"
E011_EVALUATOR_PATH = REPO_ROOT / "agent/evaluate_e011_e010_churn_audit.py"
E011_INPUT_MANIFEST_SHA256 = "bb53ccee8a2560b43c1a7a31c48611e43b84adf15723d28f3e2bb02cf4ab5c2f"
E010_FROZEN_CHURN_SHA256 = "99fe664d809fa8689749490905188957fdd176a1648c2518bdc8774b456a508d"
...
def _rows(payload: dict[str, Any], *, attempt: str) -> list[dict[str, Any]]:
    if payload.get("schema_version") != "e010-prospective-observations/v1":
...
def raw_judgment_binding_errors(
    ...
    *,
    ...
    for call in calls:
        ...
        if not isinstance(direct, dict) or str(direct.get("option") or "") != option:
        ...
        if isinstance(direct, dict) and "evidence_refs" in direct:
            errors.append(f"{prefix}: reference-free schema contains evidence_refs")
    return errors
...
def load_bundle(
    ...
    try:
        ...
        retry_policy = receipt.get("max_retries_per_logical_call")
        if int(retry_policy if retry_policy is not None else -1) != 0:
            errors.append(f"{attempt}: receipt retry policy mismatch")
        if len(calls) != 60:
            errors.append(f"{attempt}: logical call count differs from 60")
        if physical_attempt_count != 60:
            errors.append(f"{attempt}: physical attempt total differs from 60")
        for call in calls:
            qid = str((call.get("context") or {}).get("qid") or "")
            option = str((call.get("context") or {}).get("option_key") or "")
            call_attempts = call.get("attempts") or []
            if not isinstance(call_attempts, list) or len(call_attempts) != 1:
                errors.append(f"{attempt}:{qid}:{option}: attempts differ from 1")
                continue
...
def pair_anchor_errors(
    ...
    if not PRIMARY_CLAIM_PATH.is_file():
        ...
        if (
            ...
            != "e010-prospective-primary-claim/v1"
            or primary_claim.get("experiment_id") != "E010"
    ...
    if (
        claim.get("schema_version") != "e010-prospective-repeat-claim/v1"
        or claim.get("experiment_id") != "E010"
...
def evaluate_churn(
    ...
    *,
    ...
    return {
        "schema_version": "e011-e010-churn-audit/v1",
        "experiment_id": "E011",
        "audit_id": E011_AUDIT_ID,
        "audit_target_pair_id": PAIR_ID,
        ...
        "artifact_bindings": {
            ...
            "selection_audit_sha256": FROZEN_INPUT_SHA256["selection_audit"],
            "selection_sha256": FROZEN_INPUT_SHA256["selection"],
...
def _validate_audit_freeze() -> dict[str, Any]:
    freeze = _load(E011_RUN_FREEZE_PATH)
    expected_paths = {
        "audit_claim": display_path(E011_AUDIT_CLAIM_PATH),
        "audit_report": display_path(E011_AUDIT_REPORT_PATH),
        "labels": display_path(E011_LABELS_PATH),
        "scored_result": display_path(E011_SCORED_RESULT_PATH),
    }
    errors: list[str] = []
    if (
        freeze.get("schema_version") != "e011-audit-run-freeze/v1"
    ...
    if freeze.get("evaluator") != {
        ...
        errors.append("E011 evaluator hash mismatch")
    if freeze.get("frozen_input_manifest") != {
        "path": display_path(E011_INPUT_MANIFEST_PATH),
        "sha256": E011_INPUT_MANIFEST_SHA256,
    }:
        errors.append("E011 input manifest binding mismatch")
    if freeze.get("registered_paths") != expected_paths:
        errors.append("E011 registered paths mismatch")
    if freeze.get("zero_value_policy") != {
        "field": "max_retries_per_logical_call",
        "required_value": 0,
        "missing_sentinel": -1,
        "expression": "int(value if value is not None else -1)",
    }:
        errors.append("E011 zero-value policy mismatch")
    nonce = str(freeze.get("audit_nonce") or "")
    if len(nonce) != 32 or any(ch not in "0123456789abcdef" for ch in nonce):
        errors.append("E011 audit nonce invalid")
    if freeze.get("initial_state") != {
        "audit_claim_exists": False,
        "audit_report_exists": False,
        "labels_created": False,
        "scored_result_exists": False,
    }:
        errors.append("E011 initial-state mismatch")
    if freeze.get("candidate_authorized") is not False or freeze.get(
        "submission_authorized"
    ) is not False:
        errors.append("E011 freeze must not authorize candidate/submission")
    manifest = _load(E011_INPUT_MANIFEST_PATH)
    if manifest.get("status") != "FROZEN" or manifest.get("labels_created") is not False:
        errors.append("E011 input manifest semantics mismatch")
    actual = {
        "e010_pair_result": sha256_file(
            E011_EXPERIMENT_DIR.parent
            / "E010_multi_trace_evidence_binding/prospective_pair_result.json"
        ),
        "e010_run_freeze": sha256_file(RUN_FREEZE_PATH),
        "e010_selection": sha256_file(PROSPECTIVE_SELECTION_PATH),
        "e010_primary_claim": sha256_file(PRIMARY_CLAIM_PATH),
        "e010_repeat_claim": sha256_file(REPEAT_CLAIM_PATH),
        "e010_frozen_churn": sha256_file(E010_FROZEN_CHURN_PATH),
        "primary_observations": sha256_file(EXPECTED_OUTPUT_DIRS["primary"] / "observations.json"),
        "primary_receipt": sha256_file(EXPECTED_OUTPUT_DIRS["primary"] / "run_receipt.json"),
        "repeat_observations": sha256_file(EXPECTED_OUTPUT_DIRS["repeat"] / "observations.json"),
        "repeat_receipt": sha256_file(EXPECTED_OUTPUT_DIRS["repeat"] / "run_receipt.json"),
    }
    if manifest.get("sha256") != actual:
        errors.append("E011 immutable E010 input hashes mismatch")
    frozen_churn = _load(E010_FROZEN_CHURN_PATH)
    if (
        frozen_churn.get("status") != "FAIL"
        or frozen_churn.get("bundle_errors")
        != [
            "primary: receipt retry policy mismatch",
            "repeat: receipt retry policy mismatch",
        ]
    ):
        errors.append("E010 frozen churn failure is not the registered zero-value case")
    if errors:
        raise ValueError(f"E011 audit freeze/input invalid: {errors}")
    return freeze


def _claim_audit(freeze: dict[str, Any]) -> str:
    claim = {
        "schema_version": "e011-audit-claim/v1",
        "experiment_id": "E011",
        "audit_id": E011_AUDIT_ID,
        "status": "AUDIT_ATTEMPT_CLAIMED",
        "audit_nonce": freeze["audit_nonce"],
        "run_freeze_sha256": sha256_file(E011_RUN_FREEZE_PATH),
        "input_manifest_sha256": E011_INPUT_MANIFEST_SHA256,
        "e010_frozen_churn_sha256": E010_FROZEN_CHURN_SHA256,
        "evaluator_sha256": sha256_file(E011_EVALUATOR_PATH),
        "claimed_at": now_iso(),
    }
    with E011_AUDIT_CLAIM_PATH.open("x", encoding="utf-8") as handle:
        handle.write(json.dumps(claim, ensure_ascii=False, indent=2) + "\n")
    return sha256_file(E011_AUDIT_CLAIM_PATH)


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.parse_args()
    if E011_AUDIT_REPORT_PATH.exists() or E011_AUDIT_CLAIM_PATH.exists():
        raise ValueError("fixed E011 audit claim/report slot is already occupied")
    if (
        E011_LABELS_PATH.exists()
        or E011_SCORED_RESULT_PATH.exists()
        or E010_LABELS_PATH.exists()
        or E010_SCORED_RESULT_PATH.exists()
    ):
        raise ValueError("labels/scored result exist before E011 label-free audit")
    freeze = _validate_audit_freeze()
    E011_OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    allowed_reads = (
        ...
        E010_FROZEN_CHURN_PATH.resolve(),
        E011_RUN_FREEZE_PATH.resolve(),
        E011_INPUT_MANIFEST_PATH.resolve(),
        E011_AUDIT_CLAIM_PATH.resolve(),
        *default_runtime_read_roots(),
    )
    try:
        with blind_data_guard(
            ...
            allowed_write_roots=(
                E011_AUDIT_CLAIM_PATH.resolve(),
                E011_AUDIT_REPORT_PATH.resolve(),
            ),
            block_subprocess=True,
        ):
            audit_claim_sha256 = _claim_audit(freeze)
            primary = load_bundle(
            ...
            report["audit_claim_sha256"] = audit_claim_sha256
            report["audit_run_freeze_sha256"] = sha256_file(E011_RUN_FREEZE_PATH)
            report["e010_frozen_churn_sha256"] = E010_FROZEN_CHURN_SHA256
            report["zero_value_policy_applied"] = True
            rendered = json.dumps(report, ensure_ascii=False, indent=2) + "\n"
            with E011_AUDIT_REPORT_PATH.open("x", encoding="utf-8") as handle:
