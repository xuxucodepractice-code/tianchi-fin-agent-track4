"""Validate E013 full expansion and freeze a three-file candidate bundle."""
...
def _write_rerun_bundle(
    ...
    manifest = {
        ...
        "requested_scope": "e013_full_multi_65",
    ...
    if not report["ok"] or report["question_count"] != QUESTION_COUNT or report["total_tokens"] != int(receipt["total_tokens"]):
        raise ValueError(f"E013 rerun bundle validation failed: {report}")
...
def _audit_candidate(
    ...
    return {
        "schema_version": "e013-candidate-audit/v1",
        "experiment_id": EXPERIMENT_ID,
...
def main() -> int:
    ...
    if occupied:
        raise ValueError(f"E013 candidate one-shot slots occupied: {[display_path(path) for path in occupied]}")
    _verify_parent()
    ...
    if audit["status"] != "PASS":
        ...
        return 1
    result = {
        "schema_version": "e013-full-multi-result/v1",
        "experiment_id": EXPERIMENT_ID,
        "status": "PASS",
    ...
    freeze = {
        "schema_version": "e013-candidate-freeze/v1",
        "experiment_id": EXPERIMENT_ID,
