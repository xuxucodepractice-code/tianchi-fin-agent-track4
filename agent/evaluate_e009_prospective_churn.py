"""Validate E009 prospective primary/repeat and compute label-free churn."""
...
def _rows(payload: dict[str, Any], *, attempt: str) -> list[dict[str, Any]]:
    if payload.get("schema_version") != "e009-prospective-observations/v1":
...
def load_bundle(
    ...
    try:
        ...
        if int(receipt.get("max_retries_per_logical_call") or -1) != 0:
...
def pair_anchor_errors(
    ...
    if not PRIMARY_CLAIM_PATH.is_file():
        ...
        if (
            ...
            != "e009-prospective-primary-claim/v1"
            or primary_claim.get("experiment_id") != "E009"
    ...
    if (
        claim.get("schema_version") != "e009-prospective-repeat-claim/v1"
        or claim.get("experiment_id") != "E009"
...
def evaluate_churn(
    ...
    *,
    ...
    return {
        "schema_version": "e009-prospective-churn/v1",
        "experiment_id": "E009",
        "pair_id": PAIR_ID,
        ...
        "artifact_bindings": {
            ...
            "selection_feasibility_sha256": FROZEN_INPUT_SHA256["selection_feasibility"],
            "selection_sha256": FROZEN_INPUT_SHA256["selection"],
