"""Run one governed fresh E009 development arm without retries."""
...
FROZEN_INPUT_SHA256 = {
    ...
    "selection": "569e4467eb4827fe09ec49a41a498e460cba2abc7cfffbffed44d612b07a487d",
...
def _load_selection(path: Path) -> dict[str, Any]:
    if path.resolve() != SELECTION_PATH.resolve():
        raise ValueError("E009 development must use the registered selection path")
    if sha256_file(path) != FROZEN_INPUT_SHA256["selection"]:
        raise ValueError("E009 development selection SHA256 mismatch")
    payload = _load_json(path)
    if (
        payload.get("schema_version") != "e009-development-selection/v1"
        ...
        raise ValueError("E009 development selection semantics mismatch")
    return payload
...
def _verify_frozen_inputs(selection_path: Path) -> dict[str, dict[str, Any]]:
    ...
    if mismatch:
        raise ValueError(f"E009 frozen input SHA256 mismatch: {mismatch}")
...
def validate_run_freeze_payload(
    ...
    if payload.get("schema_version") != "e009-development-run-freeze/v1":
...
def _load_run_freeze() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if errors:
        raise ValueError(f"E009 development run-freeze invalid: {errors}")
...
def _validate_questions(qids: list[str], questions: dict[str, dict[str, Any]]) -> None:
    ...
    if invalid:
        raise ValueError(f"E009 requires governed A/B/C/D Multi questions: {invalid}")
...
def _control_anchor() -> dict[str, str]:
    ...
    for path in (observations_path, receipt_path, TREATMENT_AUTHORIZATION_PATH):
        if not path.is_file():
            raise ValueError(f"E009 treatment requires completed control artifact: {path}")
    ...
    if not report.get("ok") or receipt.get("status") != "PASS":
        raise ValueError("E009 registered control no longer passes Trace/receipt validation")
    anchor = {
        "pair_id": PAIR_ID,
    ...
    if (
        authorization.get("schema_version") != "e009-treatment-authorization/v1"
        or authorization.get("status") != "AUTHORIZED_FOR_ONE_TREATMENT_ATTEMPT"
        or authorization.get("control_anchor") != anchor
    ):
        raise ValueError("E009 treatment authorization does not bind current control")
    return anchor


def _claim_treatment(control_anchor: dict[str, str]) -> str:
    claim = {
        "schema_version": "e009-treatment-claim/v1",
        "experiment_id": "E009",
        "pair_id": PAIR_ID,
...
def _write_treatment_authorization(anchor: dict[str, str]) -> None:
    payload = {
        "schema_version": "e009-treatment-authorization/v1",
        "experiment_id": "E009",
        "pair_id": PAIR_ID,
...
def _trace_contract_errors(
    ...
    if completed and (len(calls) != 52 or len(derivations) != 13):
        errors.append("completed E009 arm must have 52 calls and 13 derivations")
    for call in calls:
        ...
        if context.get("stage") != f"e009_{arm}_option_judgment":
...
def main() -> int:
    ...
    if args.output_dir.resolve() != EXPECTED_OUTPUT_DIRS[args.arm].resolve():
        raise ValueError(f"E009 arm must use registered output slot: {EXPECTED_OUTPUT_DIRS[args.arm]}")
    ...
    if not doc_meta:
        raise ValueError("E009 freezes E006 treatment and requires doc_meta")
    ...
    if client.model != REQUESTED_MODEL or client.max_retries != 0:
        raise ValueError("E009 model/retry configuration drift")
    if client.base_url.rstrip("/") != OFFICIAL_DASHSCOPE_BASE_URL:
        raise ValueError("E009 requires the official DashScope endpoint")
    ...
    trace_dir = args.output_dir / "agent_traces" / f"e009-{args.arm}-{uuid.uuid4()}"
    allowed_read_roots = (
    ...
    try:
        with blind_data_guard(
            ...
            recorder = AgentTraceRecorder(
                trace_dir,
                purpose=f"e009_development_{args.arm}",
                model=client.model,
            ...
            payload = {
                "schema_version": "e009-development-observations/v1",
                "experiment_id": "E009",
                "pair_id": PAIR_ID,
                "phase": "development",
                "arm": args.arm,
    ...
    receipt = {
        "schema_version": "e009-development-run-receipt/v1",
        "experiment_id": "E009",
        "pair_id": PAIR_ID,
        "phase": "development",
        "arm": args.arm,
    ...
    print(
        f"E009 development/{args.arm}=PASS questions=13 calls=52 "
