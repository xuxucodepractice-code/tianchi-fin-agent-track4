"""Run the one-shot governed E012 expansion over all 65 Multi questions."""
...
EXPERIMENT_ID = "E012"
RUN_ID = "e012-full-multi-01"
EXPERIMENT_DIR = (
    REPO_ROOT / "workspace/03_baseline_improvement/experiments/E012_full_multi_expansion"
)
...
OUTPUT_ROOT = REPO_ROOT / "outputs/experiments/E012_full_multi_expansion"
...
CANDIDATE_DIR = REPO_ROOT / "outputs/candidates/e012_full_multi_candidate"
CANDIDATE_AUDIT_PATH = EXPERIMENT_DIR / "candidate_audit.json"
CANDIDATE_FREEZE_PATH = EXPERIMENT_DIR / "candidate_freeze.json"
RUNNER_PATH = REPO_ROOT / "agent/run_e012_full_multi.py"
BUILDER_PATH = REPO_ROOT / "agent/build_e012_candidate.py"
...
FROZEN_INPUT_SHA256 = {
    ...
    "selection": "b6fc9e0707ebe5d3ef71cf8a522b259e668358a5e5556ff48926a5f26eaa7e2e",
    "authorization": "8095b9b872ffcd9434c97238b866a82245aa2458cbf2706329e0036e94c247a0",
...
def validate_run_freeze_payload(
    ...
    errors: list[str] = []
    if (
        payload.get("schema_version") != "e012-full-multi-run-freeze/v1"
        or payload.get("freeze_id") != "E012_FULL_MULTI_01_2026-07-18"
...
def load_and_verify_run_freeze() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if errors:
        raise ValueError(f"E012 run-freeze invalid: {errors}")
...
def _load_selection() -> dict[str, Any]:
    if sha256_file(SELECTION_PATH) != FROZEN_INPUT_SHA256["selection"]:
        raise ValueError("E012 selection hash mismatch")
    payload = _load_json(SELECTION_PATH)
    qids = tuple(map(str, payload.get("qids") or []))
    if (
        payload.get("schema_version") != "e012-full-multi-selection/v1"
        or payload.get("experiment_id") != EXPERIMENT_ID
        or payload.get("selection_id") != "e012-official-group-a-multi-65"
        ...
        raise ValueError("E012 selection semantics mismatch")
    return payload
...
def _verify_frozen_inputs() -> dict[str, dict[str, Any]]:
    ...
    if mismatches:
        raise ValueError(f"E012 frozen input mismatch: {mismatches}")
    ...
    if (
        ...
        raise ValueError("E012 technical authorization mismatch")
    ...
    if (
        ...
        raise ValueError("E011 scored result does not authorize E012")
...
def _claim_payload(run_freeze: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "e012-full-multi-claim/v1",
...
def validate_full_multi_trace_contract(
    ...
    expected_config = {
        "runner": "agent.run_e012_full_multi",
        "experiment_id": EXPERIMENT_ID,
    ...
    if config != expected_config:
        errors.append("trace config differs from frozen E012 config")
    try:
        ...
        return [*errors, f"unable to inspect E012 trace: {exc}"]
...
def main() -> int:
    ...
    if args.selection.resolve() != SELECTION_PATH.resolve() or args.output_dir.resolve() != OUTPUT_DIR.resolve():
        raise ValueError("E012 must use its registered selection and output slot")
    if any(path.exists() for path in (
        ...
        raise ValueError("E012 one-shot output/claim/candidate slot is already occupied")
    if os.environ.get("SSL_CERT_FILE") != str(TLS_CA_BUNDLE_PATH):
        raise ValueError("E012 requires SSL_CERT_FILE=/etc/ssl/cert.pem")
    if sha256_file(TLS_CA_BUNDLE_PATH) != TLS_CA_BUNDLE_SHA256:
        raise ValueError("E012 TLS CA bundle hash mismatch")
    try:
    ...
    if (
        ...
        raise ValueError("E012 Qwen client identity/config mismatch")
    trace_dir = OUTPUT_DIR / "agent_traces" / f"e012-full-multi-{uuid.uuid4()}"
    ...
    try:
        with blind_data_guard(
            ...
            if any(questions[qid].get("answer_format") != "multi" for qid in qids):
                raise ValueError("E012 selection contains a non-Multi question")
            chunks = load_chunks()
            doc_meta = load_doc_meta()
            if not doc_meta:
                raise ValueError("E012 requires deterministic doc_meta")
            ...
            recorder = AgentTraceRecorder(
                trace_dir, purpose="e012_full_multi_expansion", model=client.model,
            ...
            payload = {
                "schema_version": "e012-full-multi-observations/v1",
                "experiment_id": EXPERIMENT_ID,
                "phase": "full_multi_expansion",
                "run_id": RUN_ID,
                "attempt_nonce": str(run_freeze.get("attempt_nonce") or ""),
    ...
    receipt = {
        "schema_version": "e012-full-multi-run-receipt/v1",
        "experiment_id": EXPERIMENT_ID,
        "phase": "full_multi_expansion",
        "run_id": RUN_ID,
        "attempt_nonce": str(run_freeze.get("attempt_nonce") or ""),
    ...
    if receipt["status"] != "PASS" or inventory_errors:
        for error in [*receipt["errors"], *inventory_errors]:
            print(f"[e012-full-multi-error] {error}")
        return 1
    print(
        f"E012 full Multi=PASS questions={QUESTION_COUNT} calls={LOGICAL_CALL_COUNT} "
