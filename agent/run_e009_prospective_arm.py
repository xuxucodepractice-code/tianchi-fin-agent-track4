"""Run one governed, label-blind E009 prospective primary/repeat arm."""
...
from agent.reason_e007_reference_integrity import (
    E007_PIPELINE_VERSION as E009_PIPELINE_VERSION,
    PROMPT_PROFILE,
...
PAIR_ID = "e009-prospective-pair-01"
PROSPECTIVE_SELECTION_PATH = EXPERIMENT_DIR / "prospective_selection.json"
FEASIBILITY_PATH = EXPERIMENT_DIR / "prospective_selection_feasibility.json"
AUTHORIZATION_PATH = EXPERIMENT_DIR / "prospective_quota_authorization.json"
DEVELOPMENT_RESULT_PATH = EXPERIMENT_DIR / "development_result.json"
RUN_FREEZE_PATH = EXPERIMENT_DIR / "prospective_run_freeze.json"
OUTPUT_ROOT = REPO_ROOT / "outputs" / "experiments" / "E009_multi_document_order_binding"
...
RUNNER_PATH = REPO_ROOT / "agent" / "run_e009_prospective_arm.py"
CHURN_EVALUATOR_PATH = REPO_ROOT / "agent" / "evaluate_e009_prospective_churn.py"
SCORED_EVALUATOR_PATH = REPO_ROOT / "agent" / "evaluate_e009_prospective_scored.py"
...
FROZEN_INPUT_SHA256 = {
    ...
    "selection_feasibility": "444362466cde4b2148833f4c06ea6c7f4e0fd7b3142622ac64b0b35dbd45e493",
    "selection": "8e6c0efbec5f9c8e53f3253f44d3a7046b7538e95e923516c880a987b6ba0e15",
    "authorization": "f3ee2f7b65c1fa1d79e2388ac1c6b25db89a0231d4ae6ce4fe970adf443d6e3e",
    "development_result": "6db056d5f81844edf3e93dfeef2f1470cf647f07fa7a8b0319f31ccb3ccdda4a",
...
def validate_run_freeze_payload(
    ...
    *,
    ...
    if payload.get("schema_version") != "e009-prospective-run-freeze/v1":
        errors.append("run freeze schema mismatch")
    if payload.get("freeze_id") != "E009_PROSPECTIVE_PAIR_01_2026-07-18":
        errors.append("run freeze id mismatch")
    if (
        payload.get("experiment_id") != "E009"
    ...
    if payload.get("authorization_basis") != {
        ...
        "required_status": "AUTHORIZED_MINIMAL_QUOTA_RELAXATION",
    }:
    ...
    if payload.get("pipeline_version") != E009_PIPELINE_VERSION:
        errors.append("run freeze pipeline mismatch")
    if payload.get("pipeline") != {
        "pipeline_version": E009_PIPELINE_VERSION,
...
def _load_and_verify_run_freeze() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if errors:
        raise ValueError(f"E009 prospective run freeze invalid: {errors}")
...
def _load_selection(path: Path) -> dict[str, Any]:
    ...
    if (
        payload.get("schema_version") != "e009-prospective-selection/v1"
        or payload.get("experiment_id") != "E009"
        ...
        or eligibility.get("quota_authorization_sha256")
        != FROZEN_INPUT_SHA256["authorization"]
        or payload.get("labels_created") is not False
        or payload.get("labels_revealed_to_generation") is not False
    ):
        raise ValueError("E009 prospective selection semantics mismatch")
    return payload
...
def _verify_frozen_inputs(selection_path: Path) -> dict[str, dict[str, Any]]:
    paths = {
        ...
        "selection_feasibility": FEASIBILITY_PATH,
        "selection": selection_path,
        "authorization": AUTHORIZATION_PATH,
        "development_result": DEVELOPMENT_RESULT_PATH,
    ...
    if mismatches:
        raise ValueError(f"E009 prospective frozen input mismatch: {mismatches}")

    authorization = _load_json(AUTHORIZATION_PATH)
    if (
        authorization.get("status") != "AUTHORIZED_MINIMAL_QUOTA_RELAXATION"
        ...
        or (authorization.get("constraints_preserved") or {}).get("labels_created")
        is not False
        or (authorization.get("constraints_preserved") or {}).get("labels_revealed")
        is not False
    ...
    if (
        ...
        raise ValueError("E009 development result does not authorize prospective")
...
def validate_prospective_trace_contract(
    ...
    *,
    ...
    if config.get("runner") != "agent.run_e009_prospective_arm":
        errors.append("prospective runner identity mismatch")
    if config.get("experiment_id") != "E009" or config.get("phase") != "prospective":
    ...
    if config.get("pipeline_version") != E009_PIPELINE_VERSION:
...
def _primary_claim_payload(run_freeze: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "e009-prospective-primary-claim/v1",
        "experiment_id": "E009",
        "pair_id": PAIR_ID,
        "attempt": "primary",
...
def _claim_repeat_once(
    ...
    *,
    ...
    claim = {
        "schema_version": "e009-prospective-repeat-claim/v1",
        "experiment_id": "E009",
        "pair_id": PAIR_ID,
        "attempt": "repeat",
...
def main() -> int:
    ...
    if args.output_dir.resolve() != EXPECTED_OUTPUT_DIRS[args.attempt].resolve():
        raise ValueError(
            f"E009 {PAIR_ID}/{args.attempt} must use {EXPECTED_OUTPUT_DIRS[args.attempt]}"
        )
    if os.environ.get("SSL_CERT_FILE") != str(TLS_CA_BUNDLE_PATH):
        raise ValueError("E009 requires SSL_CERT_FILE=/etc/ssl/cert.pem")
    if sha256_file(TLS_CA_BUNDLE_PATH) != TLS_CA_BUNDLE_SHA256:
        raise ValueError("E009 TLS CA bundle SHA256 mismatch")
    try:
        client = QwenClient(
    ...
    if client.model != REQUESTED_MODEL:
        raise ValueError(f"E009 freezes qwen-plus, got {client.model!r}")
    if client.base_url.rstrip("/") != OFFICIAL_DASHSCOPE_BASE_URL:
        raise ValueError("E009 requires official DashScope")
    if client.max_retries != CLIENT_MAX_RETRIES or float(
        client.timeout
    ) != CLIENT_TIMEOUT_SECONDS:
        raise ValueError("E009 prospective freezes max_retries=0 and timeout=90s")
    observations_path = args.output_dir / "observations.json"
    trace_dir = (
        args.output_dir
        / "agent_traces"
        / f"e009-prospective-{args.attempt}-{uuid.uuid4()}"
    )
    allowed_read_roots = [
        (REPO_ROOT / "agent").resolve(),
        (REPO_ROOT / "public_dataset_upload" / "questions" / "group_a").resolve(),
    ...
    try:
        with blind_data_guard(
            ...
            if not doc_meta:
                raise ValueError("E009 prospective requires deterministic doc_meta")
            ...
            config = {
                "runner": "agent.run_e009_prospective_arm",
                "experiment_id": "E009",
                "phase": "prospective",
                "pair_id": PAIR_ID,
            ...
            recorder = AgentTraceRecorder(
                trace_dir,
                purpose=f"e009_prospective_{args.attempt}",
                model=client.model,
            ...
            payload = {
                "schema_version": "e009-prospective-observations/v1",
                "experiment_id": "E009",
                "phase": "prospective",
                "pair_id": PAIR_ID,
                "attempt": args.attempt,
    ...
    receipt = {
        "schema_version": "e009-prospective-run-receipt/v1",
        "experiment_id": "E009",
        "phase": "prospective",
        "pair_id": PAIR_ID,
        "attempt": args.attempt,
    ...
    if receipt["status"] != "PASS":
        for error in receipt["errors"]:
            print(f"[e009-prospective-error] {error}")
        return 1
    print(
        f"E009 prospective {args.attempt}=PASS questions=15 calls=60 "
