...
import agent.run_e013_full_multi as e013
...
def _valid_freeze() -> dict:
    return {
        "schema_version": "e013-full-multi-run-freeze/v1",
        "freeze_id": "E013_FULL_MULTI_01_2026-07-18",
...
def test_e013_selection_is_exact_official_multi_partition() -> None:
...
def test_e013_run_freeze_contract_accepts_exact_payload() -> None:
    assert validate_run_freeze_payload(
        _valid_freeze(), current_code_snapshot=code_snapshot()
    ) == []


def test_e013_run_freeze_rejects_model_or_qid_drift() -> None:
...
def test_e013_precreated_output_root_allows_exact_claim_write(
    tmp_path, monkeypatch
) -> None:
    output_root = tmp_path / "outputs" / "E013"
    output_dir = output_root / "full_multi_01"
    claim_path = output_root / "full_multi_claim.json"
    freeze_path = tmp_path / "full_multi_run_freeze.json"
    freeze_path.write_text("{}\n", encoding="utf-8")
    output_root.mkdir(parents=True, exist_ok=False)
    monkeypatch.setattr(e013, "OUTPUT_ROOT", output_root)
    monkeypatch.setattr(e013, "OUTPUT_DIR", output_dir)
    monkeypatch.setattr(e013, "CLAIM_PATH", claim_path)
    monkeypatch.setattr(e013, "RUN_FREEZE_PATH", freeze_path)
    run_freeze = {"attempt_nonce": "a" * 32}
    with blind_data_guard(
        (tmp_path / "forbidden",),
        allowed_read_roots=(
            (e013.REPO_ROOT / "agent").resolve(),
            freeze_path.resolve(),
            output_root.resolve(),
        ),
        ...
        claim_sha256 = e013._claim_once(run_freeze)
    claim = json.loads(claim_path.read_text(encoding="utf-8"))
    assert len(claim_sha256) == 64
    assert claim["experiment_id"] == "E013"
    assert claim["run_id"] == "e013-full-multi-01"
